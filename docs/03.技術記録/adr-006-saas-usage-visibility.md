...
## 追記: 残量（使用量）の自動更新タイミング

決定 3（アプリ側の使用量取得と表示）を、ユーザー要望「アプリ側の容量更新は、アプリ起動時・
文字起こし・LLM 要約した時のみで更新してほしい」を踏まえて次のとおり明確化する。

- **自動更新は以下の 2 タイミングに限定する**。
  1. **アプリ起動時**: `MainWindow.__init__` 末尾の `refresh_usage()`（従来どおり維持）。
  2. **文字起こし・LLM 要約完了時**: 録画停止後の
     `recorder_controller._on_transcribe_complete` が「文字起こし → LLM 後処理（要約）→
     メモ保存」をすべて完了した後に GUI スレッドで呼ぶ完了コールバック
     `MainWindow._on_memo_saved()` の中で `refresh_usage()` を呼ぶ。文字起こし（Transcribe）と
     要約（Bedrock）は同一パイプラインで連続実行され、それぞれのメトリクスに反映されるため、
     完了後にまとめて **1 回だけ** 取得することで最新値を反映しつつ二重取得を避ける。
     録画パイプライン側のロジックは変更せず、既存の完了コールバック結線を利用するのみとする。
- **補助的に残す挙動（ユーザーの言う 3 タイミングには含めないが、利便性・実用性のため維持）**。
  - **手動更新ボタン「残量更新」**（`_usage_refresh_btn`, `command=refresh_usage`）: 任意の
    タイミングで最新値を確認できる手動手段として残す。削除はユーザーから明示されていない。
  - **AWS 設定変更時の再取得**（`update_aws_settings()` 経由）: 使用量 API エンドポイントが
    変わった直後に表示を更新できないと、未設定→設定直後に残量が反映されず実用上不便なため
    残す。設定変更は稀なイベントである。
- 取得はネットワーク I/O のため daemon スレッドで実行し `root.after` で GUI 反映する既存
  パターンを維持する。`_on_memo_saved` は GUI スレッドから呼ばれるため、その中から
  `refresh_usage` を呼ぶことは既存パターンと整合する。
